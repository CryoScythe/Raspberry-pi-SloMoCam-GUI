# SloMo Camera GUI for Raspberry Pi

#-->> Note: Script must be run as root by a camera app or custom aliasing in bash or zsh (for more info see http://tldp.org/LDP/abs/html/aliases.html) to support fade mgmt., brittling, framebuffer, etc.
#
# Supported versions:
#   Raspberry Pi Rev. 1 (UK)(version xx9x)
#   Raspberry Pi Rev. 1 (US)(version xx7x)
#   Raspberry Pi Rev. 1 Model B+
#   Raspberry Pi Rev. 1 Model A
#   Raspberry Pi Rev. 2
#   Raspberry Pi Rev. 2 Model B
#   Raspberry Pi Rev. 3
#   Raspberry Pi Rev. 3 Model B(version 11b1)
#   Raspberry Pi Mini [Testing]
#   Additional Versions added in Github page
#
# Prerequisites: Setup Pi and TFT module
#
# slomo.py by CryoScythe
# idea and structure based on cam.py by Phil Burgess / Paint Your Dragon for Adafruit Industries.
# BSD license, all text above must be included in any redistribution.

#Issues are listed as <<<<<-----Global Warming----->>>>>
...
#   Basic Options:
#   > Pad Rectangle ((X,Y,W,H) in pixels)
#   > $ background color and/or Icon (or None), always centered
#   > $ foreground Icon, always centered
#   > $ single callback function
#   > $ single value passed to callback
...
def timeLapse():
	...
	for i in range( 1 , v['Images'] + 1 ):
		...
		# disable the backlight, critical for night timelapses, also saves power(thanks aurobauro27!)
...
# Additional Functionality (Add more functions here!!) ...............................
...
# One-time load all icons at startup.
for file in os.listdir(iconPath):
  if fnmatch.fnmatch(file, '*.png'):
    icons.append(Icon(file.split('.')[0]))

# Relational assign buttons their icon face
print"Assigning Buttons"
for s in buttons:        # For each screenful of buttons...
  for b in s:            #  For each button on screen...
    for i in icons:      #   For each icon...
      if b.bg == i.name: #    Comparre names; If match
